...
export const createERCDepositData = (
  ...
  const data = web3.eth.abi.encodeParameters(
    ['uint8', 'uint256', 'address', 'bytes32'],
    [tokenId, tokenAmount, senderAddress, findoraTo],
  );
...
export const signature = async (data: string, address: string): Promise<string> => {
  const { web3 } = Sdk.environment;
...
export const sign = async (data: string) => {
  ...
  const encryptedMessage = web3.utils.utf8ToHex(
    JSON.stringify(encrypt(encryptionPublicKey, { data }, 'x25519-xsalsa20-poly1305')),
  );

  return encryptedMessage;
};

export const decrypted = async (data: string): Promise<FindoraWallet.IWalletWrap> => {
  const account = await getDefaultAccount();
...
export const getCurrentBalance = async (): Promise<string> => {
  const { web3 } = Sdk.environment;
  const account = await getDefaultAccount();
