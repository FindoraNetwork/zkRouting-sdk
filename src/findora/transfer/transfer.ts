...
interface ISendUtxoToEvm {
  wallet: FindoraWallet.IWallet;
  convertAmount?: bigint;
  assetCode: string;
}

async function createReceivedTransferOperation(wallet, recieversInfo, assetCode, transferOp): Promise<any> {
  ...
  for (const sid of sids) {
    try {
      ...
      console.log(`Could not add decryptAssetData to utxoDataList: ${(error as Error).message}`);
      continue;
    }
  }
  ...
  for (const item of sendUtxoList) {
    ...
    try {
      ...
      throw new Error(`Can not get client asset record. Details: "${(error as Error).message}"`);
    }
    ...
    try {
      ...
      throw new Error(
        `Can not convert given sid id to a BigInt, "${item.sid}", Details - "${(error as Error).message}"`,
      );
    }
...
async function sendUtxoToEvm({ wallet, convertAmount, assetCode }: ISendUtxoToEvm) {
  const ledger = await getLedger();
...
export const barToEVM = async (
  wallet: FindoraWallet.IWallet,
  amount: string,
  ethAddress: string,
  assetCode: string,
  lowLevelData: string,
  isSourceCross: boolean,
) => {
  const ledger = await getLedger();
  const newWallet = {
    address: wallet.address,
  ...
  const params: [any, string, bigint, string?, string?] = [newWallet.keypair, ethAddress, convertAmount];
