...
export interface DeckAbiEventsContext {
  Deposit(
    parameters: {
      filter?: { sender?: string | string[]; token?: string | string[] };
      fromBlock?: number;
  ...
  Withdraw(
    parameters: {
      filter?: { to?: string | string[]; token?: string | string[] };
      fromBlock?: number;
      toBlock?: 'latest' | number;
      topics?: string[];
    },
...
export type DeckAbiMethodNames =
  | 'MaxFee'
  | 'MaxFeePercent'
  | 'MaxFeeShare'
  | 'accountAssets'
