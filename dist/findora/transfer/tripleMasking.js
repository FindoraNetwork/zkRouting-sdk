...
const getOwnedAbar = async (commitment) => {
    try {
        ...
        throw new SdkError({ errorCode: ErrorCodes.API_GET_OWNED_ABAR_ERROR, message: error.message });
    }
};
const findFeeSid = async (sids, feeAmount) => {
...
export const barToAbar = async (data, sids = []) => {
    const ledger = await getLedger();
    ...
    let keypair;
    try {
        keypair = ledger.keypair_from_str(walletStart.keyStore);
    }
    catch (error) {
    ...
    for (const utxoItem of utxos) {
        ...
        if (sid === feeSid) {
            ...
            try {
                transactionBuilder = transactionBuilder.add_operation_bar_to_abar(abarSeeds.shift(), keypair, axfrPublicKey, BigInt(sid), assetRecord, ownerMemo?.clone());
            }
            catch (error) {
                throw new SdkError({
